...
def extract_coords_from_attrs(obj: AbstractDataContainer, *,
    ...
    """Extract coordinates from the given attributes.
    ...
    Args:
        obj (AbstractDataContainer): The object to retrieve the attributes from
    ...
    def get_coord(attrs: dict, dim_name: str, dim_num: int):
        ...
        try:
            ...
            raise type(exc)("Failed extracting coordinates for dimension "
                            "'{}' via extraction mode '{}'! {}"
                            "".format(dim_name, mode, exc)) from exc
    ...
    # Done.
    # Check if strict attribute checking was enabled
    if not strict:
        # Nope, can already return
        return coords_map
    ...
    for attr_name, prefix in product(attrs.keys(), prefixes):
        ...
        if (    attr_name.startswith(prefix)
            ...
            dims_avail = [d for d in dims if d is not None]

            raise ValueError("Got superfluous attribute '{}' that does not "
                             "match any of the available names of labelled "
                             "dimensions: {}. Valid attribute prefixes: {}. "
                             "Either remove the attribute or turn strict "
                             "attribute checking off."
                             "".format(attr_name, ", ".join(dims_avail),
                                       ", ".join(prefixes)))
...
def extract_coords_from_name(obj: AbstractDataContainer, *,
    ...
    if len(coords) != len(dims):
        raise ValueError("Number of coordinates extracted from the name of {} "
                         "does not match the number of expected dimensions! "
                         "Parsed coordinates: {}. Dimensions: {}"
                         "".format(obj.logstr, coords, dims))

    if not all(coords):
        raise ValueError("One or more of the coordinates extracted from the "
                         "name of {} were empty! Got: {}"
                         "".format(obj.logstr, coords))
...
def extract_coords(obj: AbstractDataContainer, *, mode: str, dims: TDims,
    ...
    if mode not in EXTRACTORS:
        raise ValueError("Invalid extraction mode '{}'! Valid modes: {}"
                         "".format(mode, ", ".join(EXTRACTORS.keys())))
    ...
    try:
        ...
        raise type(exc)("Failed extracting coordinates of {} using "
                        "mode '{}': {}".format(obj.logstr, mode, exc)
                        ) from exc
