...
class PlotHelper:
    ...
    def __init__(self, *, out_path: str, helper_defaults: dict=None,
        ...
        if update_helper_cfg:
            self._cfg = recursive_update(self._cfg, update_helper_cfg)

        # Check that all entries are actually valid helpers
        for helper_name in self._cfg:
            if helper_name in ('setup_figure', 'save_figure'):
                continue

            elif helper_name not in self._AVAILABLE_HELPERS:
    ...
    def cfg(self) -> dict:
        ...
        This should be when it's needed to make sure that the (mutable) _cfg
        is somehow changed...
        """
    ...
    def enabled_helpers(self) -> list:
        """Returns a list of enabled helpers"""
        return [hn for hn in self._cfg
                if self._cfg[hn].get('enabled', True)
                and hn not in ('setup_figure', 'save_figure')]
    ...
    def setup_figure(self):
        ...
        self._fig = plt.figure(**self.cfg.get('setup_figure', {}))
    ...
    def invoke_helper(self, helper_name: str, *,
        ...
        Args:
            ...
            **update_kwargs: Update parameters for all plot helpers. Note that
                these do not persist, but are only used for this invocation.
        ...
        # Prepare the helper parameters, working on a copy
        helper_params = self.cfg.get(helper_name, {})
        
        if update_helper_params:
            helper_params = recursive_update(helper_params,
                                             update_helper_params)

        # If it is disabled, can return here
        if not helper_params.pop('enabled', True):
            log.debug("Helper '%s' is not enabled; not invoking it.",
                      helper_name)
            return
    ...
    def invoke_helpers(self, *helper_names,
        ...
        Args:
            ...
            **update_helper_params: Update parameters for all plot helpers.
        ...
        for helper_name in helper_names:
            self.invoke_helper(helper_name,
                               ...
                               **update_helper_params.get(helper_name, {}))

    def invoke_all(self, **update_helper_params):
    ...
    def provide_defaults(self, helper_name: str, *, mark_enabled: bool=True,
                         **update_kwargs):
        ...
        if helper_name in self.cfg:
            ...
            self._cfg[helper_name] = copy.deepcopy(update_kwargs)
    ...
    def mark_enabled(self, *helper_names):
        ...
        for helper_name in helper_names:
            self._check_helper_name(helper_name)
            self._cfg[helper_name]['enabled'] = True
