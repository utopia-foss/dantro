...
def graph_grps(graph_grp_cfgs) -> Union[dict, dict]:
    ...
    for name, cfg in graph_grp_cfgs.items():
        ...
        if name.startswith("grp"):
            ...
            grps[name].new_container('np', Cls=XrDataContainer,
                                           data=cfg.get('np',[]),
                                           dims=['time','node_idx'])

            grps[name].new_container('ep', Cls=XrDataContainer,
                                           data=cfg.get('ep',[]),
                                           dims=['time','edge_idx'])
    ...
    def basic_graph_creation_test(g, cfg):
        ...
        for e in _edges:
            if directed:
                ...
                assert (e in [edge[:2] for edge in g.edges]
                        or e in [edge[:2][::-1] for edge in g.edges])
    ...
    for name, grp in grps.items():
        ...
        if name == 'wrong_nodes':
            with pytest.raises(KeyError,
                ...
                grp.create_graph(at_time_idx=cfg.get('at_time_idx', None),
...
def test_set_property_functions(graph_grps, graph_data):
    ...
    # this should work
    grp.set_node_property(g=g, name='np',
                          at_time_idx=cfg.get('at_time_idx', None))
    grp.set_edge_property(g=g, name='ep',
                          at_time_idx=cfg.get('at_time_idx', None))
    ...
    with pytest.raises(KeyError,
                       match="No key, key sequence, or property 'foo' in "
                             "GraphGroup 'grp_prop'"):
    ...
    with pytest.raises(ValueError,
                       match="Received 6 property values for 5 nodes"):
        grp.set_node_property(g=g, name='np2',
                              at_time_idx=cfg.get('at_time_idx', None))

    with pytest.raises(ValueError,
                       match="Received 6 property values for 5 edges"):
        grp.set_edge_property(g=g, name='ep2',
                              at_time_idx=cfg.get('at_time_idx', None))
    ...
    with pytest.raises(ValueError,
                       match="Received 1 property values for 5 nodes"):
        grp.set_node_property(g=g, name='np_single',
                              at_time_idx=cfg.get('at_time_idx', None))
    ...
    with pytest.raises(ValueError,
                       match="'np2' matches the current number of nodes"):
        grp.set_node_property(g=g, name='np2',
                              at_time_idx=cfg.get('at_time_idx', None))

    with pytest.raises(ValueError,
                       match="'ep2' matches the current number of edges"):
        grp.set_edge_property(g=g, name='ep2',
                              at_time_idx=cfg.get('at_time_idx', None))
    ...
    with pytest.raises(ValueError,
                       match="might be interpreted as a single one"):
        grp.set_edge_property(g=g, name='ep2',
                              at_time_idx=cfg.get('at_time_idx', None))
    ...
    with pytest.raises(TypeError, match="Make sure that all edges in "
    ...
    gg.new_container('nodes', Cls=XrDataContainer, data=[[0,1,2]],
                              dims=('time','node_idx'))
    gg.new_container('edges', Cls=XrDataContainer, data=[[[1,0],[0,2],[2,1]]],
                              dims=('time','edge_idx'))
    gg.new_container('ep', Cls=XrDataContainer, data=[[1,0,2]],
                           dims=('time','edge_idx'))
    ...
    gg.new_container('edges', Cls=XrDataContainer,
                              data=[[3,4], [3,4], [4,3], [3,4]])
    ...
    with pytest.raises(ValueError, match="Received 3 property values for "
                                         "2 edges"):
        gg.set_edge_property(g=g, name='ep_transposed')
...
def test_loading_external_data():
    ...
    gg.new_container('nodes', Cls=XrDataContainer,
                              data=[[2,1,0]],
                              dims=('time', 'node_idx'),
    ...
    np1 = XrDataContainer(name='np1', data=[[0,0,0],[2,1,0]],
                                      dims=('time', 'node_idx'),
                                      coords=dict(time=[0,10],
                                                  node_idx=[2,1,0]))
    ep1 = XrDataContainer(name='ep1', data=[[0,0,0],[1,0,2]],
                                      dims=('time', 'edge_idx'),
                                      coords=dict(time=[0,10],
                                                  edge_idx=[1,0,2]))
    ...
    np2.new_container('0', Cls=XrDataContainer, data=np.arange(10),
                           dims=['node_idx'], coords=dict(node_idx=range(10)))
    np2.new_container('10', Cls=XrDataContainer, data=['foo','bar','baz'],
                            dims=['node_idx'], coords=dict(node_idx=[0,1,2]))
    ep2 = TimeSeriesGroup(name='ep2', dims=['time'])
    ep2.new_container('0', Cls=XrDataContainer, data=np.arange(10),
                           dims=['edge_idx'], coords=dict(edge_idx=range(10)))
    ep2.new_container('10', Cls=XrDataContainer, data=['foo','bar','baz'],
                            dims=['edge_idx'], coords=dict(edge_idx=[0,1,2]))
    ...
    gg.set_node_property(g=g, name='np3',
                         sel=dict(subdim2='coord2'), isel=dict(dim=0))
    ...
    np5 = XrDataContainer(name='np5', data=[[0,0,0],[2,1,0]],
                                      dims=('time',))
    ep5 = XrDataContainer(name='ep5', data=[[0,0,0],[1,0,2]],
                                      dims=('time',))
    ...
    with pytest.warns(UserWarning, match="No matching dimensions found"):
        gg.set_node_property(g=g, name='np5', data=np5,
                             at_time_idx=-1, align=True)

    # Aligning external data with coordinate mismatch
    np6 = XrDataContainer(name='np6', data=[[42],[24]], dims=('node_idx','foo'),
                          coords=dict(node_idx=[555,999]))
    ep6 = XrDataContainer(name='ep6', data=[[42],[24]], dims=('edge_idx','foo'),
    ...
    # Do it again, now checking for the warning
    with pytest.warns(UserWarning, match="Found missing values in property "
                                         "data"):
        gg.set_node_property(g=g, name='np6', data=np6,
                             at_time_idx=-1, align=True)
