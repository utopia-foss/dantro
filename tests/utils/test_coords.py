...
def test_coord_extractor_functions():
    ...
    assert isinstance(
        extr["linked"]("foo/bar", link_anchor_obj=C()),
        dantro.utils.coords.Link,
    )
    assert isinstance(
        extr["linked"](np.array(["foo/bar"]), link_anchor_obj=C()),
        dantro.utils.coords.Link,
    )
...
def test_extract_coords_from_name():
    ...
    exp1 = dict(foo=[123], bar=[456], baz=[789])
    assert (
        extract(Cont("123;456;789"), dims=("foo", "bar", "baz"), separator=";")
        == exp1
    )

    exp2 = dict(foo=["123"], bar=["456"], baz=["789"])
    assert (
        extract(
            Cont("123;456;789"),
            dims=("foo", "bar", "baz"),
...
def test_extract_coords():
    ...
    with pytest.raises(ValueError, match="Invalid extraction mode 'bad_mode'"):
        extract(
            XrDataContainer(name="test", data=[1, 2, 3]),
            mode="bad_mode",
            dims=("foo",),
        )
