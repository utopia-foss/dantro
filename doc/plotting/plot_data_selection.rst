...
Instead, the **best way** of implementing DAG support is via the :py:func:`~dantro.plot.creators.ext.is_plot_func` decorator.
...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
A plot function can then be defined via the following signature and the :py:func:`~dantro.plot.creators.ext.is_plot_func` decorator:
